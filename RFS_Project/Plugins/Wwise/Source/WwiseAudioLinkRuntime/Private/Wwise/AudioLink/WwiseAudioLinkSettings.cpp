...
FDelegateHandle FWwiseAudioLinkSettingsProxy::CallOnEventLoaded(const TFunction<void()>&& InCallback)
{
	{
		...
		if (!bIsEventDataLoaded)
		{
			...
			return OnEventLoadedDelegate.Add(FSimpleMulticastDelegate::FDelegate::CreateLambda(InCallback));
		}
	}
	InCallback();
